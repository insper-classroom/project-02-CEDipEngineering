...
if __name__ == "__main__":
    ...
    logger = TensorboardLogger(torch.utils.tensorboard.SummaryWriter('log/'+EXPERIMENT_NAME))
    ...
    def save_checkpoint_fn(epoch, env_step, gradient_step):
        ...
        if epoch % CHECKPOINT_FREQUENCY == 0:
            ckpt_path = os.path.join("log", "ckpt", EXPERIMENT_NAME, "policy_checkpoint_0_{:03d}.pth".format(epoch))
            ckpt_path2 = os.path.join("log", "ckpt", EXPERIMENT_NAME, "policy_checkpoint_1_{:03d}.pth".format(epoch))
            os.makedirs(os.path.join("log", "ckpt", EXPERIMENT_NAME), exist_ok=True)
